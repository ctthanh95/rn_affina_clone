...
export const getNewsDetail = createAction('GET_NEWS_DETAIL');

export function* getNewsDetailSaga(options: any) {
  const {
    type,
  ...
  try {
    function* execution() {
      const {code, message, data} = yield call(getNewsDetailApi, dataGet);
      switch (code) {
        case '200':
